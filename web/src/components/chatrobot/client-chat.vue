<template>
    <div class="client-chat" style="height: 100%">
        ...
        <div class="qa-item" v-for="(item, index) in msgList" :key="index">
            ...
            <!-- 问题 -->
            <div class="question-item" style="max-width: 80%;" v-if="item.is_from_self">
                <!-- <v-spinner status="default" class="qs-loading" v-if="item.is_loading"></v-spinner> -->
                <div class="question-text"  v-html="renderContent(item)">
            ...
            <div class="answer-item" v-if="!item.is_from_self">
                ...
                <!-- 答案信息 -->
                <div class="answer-info" style="max-width: 80%;" :ref="item.record_id">
                    <div class="loading" v-if="item.loading_message">正在思考中</div>
                    <div class="answer-md"  v-html="renderContent(item)">
...
import { scrollToBottom } from '@/utils/chatrobot/util';
...
export default {
    ...
    methods: {
        ...
        listenClientAndManageEvent () {
            // 从缓存获取机器人信息
            const cachedConfig = globalRobotData.clientData.getConfigInfo();
            if (cachedConfig) {
                this.robotName = cachedConfig.name;
            }

            // 监听答案消息队列变更事件
            if(eventHubData.eventHub) {
        ...
        listenCommonEvent () {
            ...
            eventHubData.eventHub.$on('data_history', () => {
                    this.historyLoading = false;
                });

                eventHubData.eventHub.$on('data_historyError', () => {
                    this.historyLoading = false;
                });

            eventHubData.eventHub.$on('client_replyfinal', (data) => {
              // const userid = localStorage.getItem("lke_robot_userid");
              // const sessionId = localStorage.getItem("robot_session_id");
              // httpPost("/api/lke/saveLog", {
              //       "userId": userid,
              //       "sessionId": sessionId,
              //       "recordId": data.record_id,
              //       "question": globalRobotData.clientData.getLastSendMsg() ,
              //       "answer": data.content}  ).then(res => {
              // }).catch(e => {
              // })
            }) ;
        },
